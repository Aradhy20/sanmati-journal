                    ...
                    <div className="lg:col-span-1 space-y-8">
                        <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
                            <h3 className="text-lg font-bold text-dark mb-6 flex items-center gap-2">
                                <FolderPlus className="text-primary w-5 h-5" /> New Issue
                            </h3>
                            <form onSubmit={submitIssue} className="space-y-4">
                                ...
                                <label className="flex items-center gap-2 cursor-pointer pt-2">
                                    ...
                                    />
                                    <span className="text-xs font-bold text-gray-600 uppercase tracking-widest">Mark as Current Issue</span>
                                </label>
                                <button
                        ...
                        <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
                            <h3 className="text-lg font-bold text-dark mb-6">Existing Issues</h3>
                            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                {issues.length === 0 ? (
                                    <p className="text-center text-gray-400 py-4 text-sm italic">No issues created yet.</p>
                                ) : (
                                    issues.map((issue) => (
                                        <div key={issue.id} className={`p-4 rounded-2xl border transition-all flex justify-between items-center group ${issue.is_current ? "bg-primary/5 border-blue-100" : "bg-warm-bg border-slate-100 hover:border-gray-200"
                                            }`}>
                                            <div>
                                                <p className="font-bold text-dark text-sm">Vol. {issue.volume}, No. {issue.number}</p>
                                                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{issue.month_range} {issue.year}</p>
                                            </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                            ...
                            <form onSubmit={submitPaper} className="space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Select Issue</label>
                                        <select
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Paper Category</label>
                                        <select
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Paper Title</label>
                                    <textarea
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Authors</label>
                                        <input
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">PDF File</label>
                                        <input
                                            type="file"
                                    ...
                                    <div className="md:col-span-2">
                                        <label className="flex items-center gap-2 cursor-pointer">
                                            <input
                                                ...
                                                className="w-4 h-4 rounded text-primary focus:ring-blue-500"
                                            />
                                            <span className="text-sm font-bold text-dark/80">Feature this paper on Homepage</span>
                                        </label>
                                    </div>
                                </div>
                                <button
                        ...
                        <div className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
                                <h3 className="text-lg font-bold text-dark">Recent Paper Uploads</h3>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse">
                                    <thead className="bg-warm-bg/50">
                                        <tr className="border-b border-slate-100">
                                            <th className="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Issue</th>
                                            <th className="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Paper Details</th>
                                            ...
                                            papers.map((paper) => (
                                                <tr key={paper.id} className="hover:bg-warm-bg/50 transition-colors group">
                                                    <td className="px-8 py-6 whitespace-nowrap">
                                                        ...
                                                        <p className="text-[10px] font-bold text-gray-400">{paper.issue?.year}</p>
                                                    </td>
                                                    <td className="px-8 py-6 max-w-md">
                                                        <p className="font-bold text-dark text-sm mb-1 leading-snug line-clamp-2">{paper.title}</p>
                                                        <p className="text-xs text-gray-500 line-clamp-1">{paper.authors}</p>
                                                    </td>
                                                    ...
                                                    <td className="px-8 py-6 text-right">
                                                        <div className="flex justify-end gap-2">
                                                            <a href={`/storage/${paper.file_url}`} target="_blank" className="p-2 text-primary hover:bg-primary/5 rounded-lg transition-all" title="View PDF">
