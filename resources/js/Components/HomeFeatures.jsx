            ...
            <div className={`flex flex-col md:flex-row items-center gap-12 md:gap-20 ${align === 'right' ? 'md:flex-row-reverse' : ''}`}>
                        ...
                        <div className="relative group rounded-3xl overflow-hidden shadow-2xl border border-gray-200/50 bg-white">
                            ...
                            <div className={`absolute bottom-6 ${align === 'left' ? 'right-6' : 'left-6'} bg-white/90 backdrop-blur-md p-4 rounded-xl shadow-lg border border-white/20 max-w-[200px]`}>
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <span className="font-bold text-dark text-sm">Key Benefit</span>
                                </div>
                                <p className="text-xs text-gray-600 font-medium">Optimized for maximum impact and reach.</p>
                            </div>
                        </div>
                    </ScrollReveal>
                </div>
                ...
                <div className="flex-1">
                    <ScrollReveal variants={revealVariants.fadeUp} delay={delay + 0.1}>
                        <div className="p-2 bg-primary/5 text-primary-dark rounded-lg inline-block mb-4">
                            <Icon className="w-6 h-6" />
                        </div>
                        <h3 className="text-2xl md:text-3xl font-serif font-bold text-dark mb-4 leading-tight">
                            {title}
                        </h3>
                        ...
                        <ul className="space-y-3 mb-6">
                            {[1, 2, 3].map((_, i) => (
                                <li key={i} className="flex items-start gap-3">
                                    ...
                                    <span className="text-dark/80 text-sm">Detailed feature point demonstrating value proposition {i + 1}.</span>
                                </li>
                            ))}
                        </ul>

                        <Link href="/basic-info/about-journal" className="inline-flex items-center gap-2 text-primary font-bold hover:text-primary-dark group transition-colors">
