    ...
    return (
        <section className="relative flex flex-col min-h-[100svh] overflow-hidden bg-gradient-to-br from-primary via-[#4f46e5] to-secondary">
            ...
            <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
                ...
                <div className="absolute bottom-0 right-0 w-[260px] h-[260px] sm:w-[380px] sm:h-[380px] lg:w-[500px] lg:h-[500px] bg-white/10 rounded-full blur-[120px]" />
            </div>
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 pt-32 pb-24 lg:pt-36 lg:pb-32 flex-1 flex flex-col justify-center">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-16 items-center">
                    ...
                    >
                        {/* Tagline pill */}
                        <div className="inline-flex items-center gap-2 px-5 py-2 bg-white/10 backdrop-blur-md border border-white/20 rounded-full mb-6">
                            <Sparkles className="w-3.5 h-3.5 text-accent-light" />
                            <span className="text-white font-black text-[11px] uppercase tracking-[0.35em]">
                                ISSN: 3108-1819 · Peer-Reviewed Journal
                            </span>
                        </div>

                        {/* Hindi Tagline */}
                        <p className="text-accent-light font-bold text-base mb-3 font-sans" lang="hi">
                            ज्ञान, शोध और प्रकाशन का एक राष्ट्रीय मंच
                        </p>

                        {/* Main Headline */}
                        <h1 className="text-4xl md:text-5xl lg:text-7xl font-serif font-black mb-8 leading-[1.1] tracking-tight text-white drop-shadow-md">
                            Empowering <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent-light to-white">Academic Excellence</span>
                        </h1>

                        {/* Description */}
                        <p className="text-lg md:text-xl text-white/90 font-medium leading-relaxed mb-10 max-w-lg border-l-4 border-accent-light/50 pl-6">
                            Publishing cutting-edge research across science, arts, commerce, and law. A national, peer-reviewed quarterly journal.
                        </p>

                        {/* CTA Buttons */}
                        <div className="flex flex-col sm:flex-row flex-wrap items-center gap-4 mb-12 max-w-2xl">
                            <Link
                            ...
                            >
                                <BookOpen className="w-4 h-4" /> Current Issue
                            </Link>
                        </div>
                    </motion.div>
                    ...
                    >
                        {/* Main large image — Goddess Saraswati or Concept Art */}
                        <div className="relative rounded-[2rem] overflow-hidden shadow-2xl bg-white/10 backdrop-blur-sm p-3 border border-white/20">
                            <img
                            ...
                            <div className="absolute bottom-3 left-3 right-3 p-6 bg-white/90 backdrop-blur-md rounded-2xl border border-white/50 shadow-lg">
                                <div className="flex items-center gap-3 mb-3">
                                    <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                    <span className="text-dark text-xs font-bold uppercase tracking-widest">New Submissions Open</span>
                                </div>
                                <div className="flex -space-x-3">
                                    {['https://i.pravatar.cc/36?img=3', 'https://i.pravatar.cc/36?img=5', 'https://i.pravatar.cc/36?img=7'].map((src, j) => (
                                        <img key={j} src={src} alt="Reviewer" className="w-9 h-9 rounded-full border-2 border-white object-cover shadow-sm" />
                                    ))}
                                    <div className="w-9 h-9 rounded-full border-2 border-white bg-primary flex items-center justify-center text-[10px] font-black text-white shadow-sm">+12</div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
                        {[
                            ...
                            <div key={i} className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-slate-500 text-[9px] font-black uppercase tracking-widest leading-none mb-1">{item.label}</p>
                                    <p className="text-slate-800 text-[11px] font-bold">{item.value}</p>
                                </div>
                            </div>
                        ))}
                    </div>
