            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
                {issues.length > 0 ? (
                            ...
                            <div key={issue.id} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {issue.papers && issue.papers.map((paper) => (
                                        <div key={paper.id} className="p-4 bg-warm-bg rounded-xl border border-slate-100">
                                            <h3 className="font-bold text-dark mb-2">{paper.title}</h3>
                                            <p className="text-sm text-gray-600 mb-4">{paper.authors}</p>
                                            <a href={`/storage/${paper.file_path}`} className="text-primary text-sm font-bold hover:underline">
                                                Download PDF
                                            </a>
                                        </div>
                                    ))}
                    ...
                    <div className="text-center py-20 bg-white rounded-2xl border border-slate-100">
                        <p className="text-gray-500 text-lg">No archived issues found yet.</p>
                        <p className="text-sm text-gray-400 mt-2">Check back soon for our inaugural issue!</p>
                    </div>
                )}
            </div>
        </MainLayout>
